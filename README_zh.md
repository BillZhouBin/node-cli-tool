...
- 🌍 **时区支持** - 支持 IANA 城市名和 UTC+8/GMT+2 等偏移格式
- 📊 **双精度支持** - 支持秒和毫秒级时间戳
- ✨ **自动检测** - 自动检测时间戳格式（秒/毫秒）
- 🎯 **简单 API** - 直观的命令行界面
- 🌐 **网页版** - 基于浏览器的可视化时间戳转换工具
- 📋 **JSON 工具** - 格式化/压缩 JSON，折叠视图，行号，主题切换
- 🎨 **颜色工具** - 支持解析和计算带透明度的颜色值
- 📝 **Markdown 预览** - Markdown 编辑与实时预览
...
- 可视化时间戳 ↔ 日期转换
- 实时显示当前时间戳
- 34 个时区选择（UTC-12 至 UTC+14，含半小时偏移时区）
- 时间差计算器（显示起止时间和完整天/时/分/秒格式）
- JSON 美化/压缩，折叠视图，行号，主题切换
- 颜色值解析和透明度计算
- Markdown 编辑与实时预览（支持全屏模式）
- 一键复制结果
...
1. **时间戳 ↔ 日期转换**
   - 实时显示当前时间戳
   - 34 个时区选择（UTC-12 至 UTC+14，自动识别本地时区）
   - 自定义格式支持

2. **时间差计算器**
   - 计算两个时间戳之间的时间差
   - 显示解析后的起止时间，结果包含天/时/分/秒完整格式
   - 支持的输入格式：
     - **Unix 时间戳**：`1713787200`（秒）或 `1713787200000`（毫秒）
     - **纯日期**：`2025-12-21`
     - **日期+时间**：`2024-04-22 20:00:00`
     - **斜杠格式**：`2024/04/22 20:00:00`
     - **ISO 8601**：`2025-05-21T10:30:00`

3. **JSON 工具**
   - JSON 美化（格式化输出，带缩进）
   - JSON 压缩（去除空格和换行）
   - 美化后支持折叠括号范围（`{}`、`[]`），点击 ▼ 折叠/展开
   - 行号显示，便于错误定位
   - 折叠视图 ↔ 编辑模式双向切换
   - 语法校验，错误提示精准到行号
   - 亮色/暗色主题切换
...
4. **颜色工具**
   - 解析色值字符串（支持 #ff000000 或 0xFF000000 格式）
   - 计算带透明度的颜色（支持 16 进制或百分比）
   - 显示颜色预览和 RGBA 值
...
```bash
node bin/cli.js ts2date 1713787200 -z UTC+8   # 东8区（北京时间）
node bin/cli.js ts2date 1713787200 -z UTC+2   # 东2区
node bin/cli.js ts2date 1713787200 -z UTC-5   # 西5区（纽约时间）
node bin/cli.js ts2date 1713787200 -z GMT+8   # 等同 UTC+8
```

### 3. Etc/GMT 格式

```bash
node bin/cli.js ts2date 1713787200 -z Etc/GMT-8  # 注意：符号与常规相反
```
